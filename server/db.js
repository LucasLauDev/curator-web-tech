...
async function insertRegisteredStudent (
  pool,
  { email, password, role, studentIdUpper, firstName, lastName, year, faculty, instructorTitle }
) {
  ...
  const sql = `
    ...
    VALUES (
      lower(trim($1::text)),
      CASE WHEN $4::text = 'student' THEN upper(regexp_replace(trim($2::text), '\\s+', '', 'g')) ELSE NULL END,
      crypt($3::text, gen_salt('bf')),
      $4::text,
      $5, $6, $7::smallint, trim($8::text),
      NULL, $9
    )
  ...
  const { rows } = await pool.query(sql, [
    ...
    year,
    faculty,
    normalizedRole === 'instructor' ? instructorTitle : null
  ]);
  return stripSecret(rows[0]) || null;
}
