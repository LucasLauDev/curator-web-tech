...
let databaseStatus = DATABASE_URL && pool ? 'checking' : 'disabled';
let databaseErrorMessage = '';

function makeDatabaseUnavailableError () {
  const message = databaseErrorMessage || 'Database connection is not ready. Check DATABASE_URL in .env.';
  const err = new Error(message);
  err.name = 'DatabaseUnavailableError';
  err.code = databaseStatus === 'disabled' ? 'DB_DISABLED' : 'DB_UNAVAILABLE';
  err.stack = undefined;
  return err;
}

if (pool && rawPoolQuery) {
  pool.query = async function guardedPoolQuery () {
    if (databaseStatus !== 'ready') {
      throw makeDatabaseUnavailableError();
    }
    return rawPoolQuery.apply(pool, arguments);
  };
}

async function probeDatabaseConnectivity () {
  if (!rawPoolQuery || databaseStatus === 'disabled') return;
  try {
    await rawPoolQuery('SELECT 1');
    ...
    databaseErrorMessage = host
      ? `Unable to reach ${host}. Check DATABASE_URL or network access.`
      : 'Unable to reach the database. Check DATABASE_URL or network access.';
    if (detail && !databaseErrorMessage.includes(detail)) {
      databaseErrorMessage += ` (${detail})`;
    }
    console.warn(databaseErrorMessage);
  }
}
...
function dbReady () {
  return !!(pool && DATABASE_URL && databaseStatus === 'ready');
}
...
app.get('/api/health', (_req, res) => {
  ...
  res.json({
    ...
    databaseConnected: databaseStatus === 'ready',
    databaseStatus,
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({ error: 'Registration requires a valid email address' });
  }
  if (requestedRole === 'student' && !e.endsWith('@student.uts.edu.my')) {
    return res.status(400).json({ error: 'Student registration requires a valid @student.uts.edu.my email' });
  }
  if (requestedRole === 'instructor' && !e.endsWith('@uts.edu.my')) {
    return res.status(400).json({ error: 'Instructor registration requires a valid @uts.edu.my email' });
  }
  ...
  if (requestedRole === 'student') {
    sid = String(studentId || '').replace(/\s+/g, '').toUpperCase();
    ...
    return res.status(400).json({ error: 'Instructor title is required' });
  }
...
const server = app.listen(PORT, () => {
  ...
  void probeDatabaseConnectivity();
